...
class TestArchiveAIBackend(unittest.TestCase):
    ...
    def test_14_users_cannot_delete_each_others_records(self):
        """Deletion endpoints must enforce ownership for authenticated users."""
        journalist_login = client.post("/api/auth/login", json={
            "email": "journalist@archiveai.org",
            "password": "archive2024"
        })
        researcher_login = client.post("/api/auth/login", json={
            "email": "researcher@archiveai.org",
            "password": "researcher2024"
        })
        journalist_headers = {
            "Authorization": f"Bearer {journalist_login.json()['access_token']}"
        }
        researcher_headers = {
            "Authorization": f"Bearer {researcher_login.json()['access_token']}"
        }

        document = client.get("/api/documents/ARCH-8821").json()
        saved_response = client.post(
            "/api/saved",
            json={"document_id": document["id"], "notes": "Ownership test"},
        ...
        search_response = client.get(
            "/api/search?q=renewable",
            headers=journalist_headers
        )
        self.assertEqual(search_response.status_code, 200)
        history = client.get("/api/history", headers=journalist_headers).json()
        history_id = history[0]["id"]

        self.assertEqual(
            client.delete(f"/api/saved/{saved_id}", headers=researcher_headers).status_code,
            404
        )
        self.assertEqual(
            client.delete(f"/api/history/{history_id}", headers=researcher_headers).status_code,
            404
        )

        self.assertEqual(
            client.delete(f"/api/saved/{saved_id}", headers=journalist_headers).status_code,
            200
        )
        self.assertEqual(
            client.delete(f"/api/history/{history_id}", headers=journalist_headers).status_code,
            200
        )
